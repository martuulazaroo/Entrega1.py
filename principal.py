...
def ordenamiento_por_intercambio_mejorada(lista):
    '''
...
def main():
    ...
    while True: #bucle general del programa, se repetirá hasta que el usuario elija salir (opción 0)
        ...
        if opcion == "0": # Opción salir del programa
            ...
            if not listas_datos: # Si la lista está vacía, no se pueden ordenar valores
                print("La lista está vacía. No hay datos para ordenar. Debe generar valores primero (Opción 1)")
            else:
                ordenar_valores (listas_datos)
        elif opcion == "9":
            if not listas_datos: #Si la lista está vacía, no se pueden buscar valores
